...
class Municipio {
   ...
   public function getMunicipioById($id){
    ...
    public function getEstadoMunicipio($municipioId){
        $this->db->query('SELECT estados.id, estados.regiaoId, estados.estado FROM estados, municipios WHERE estados.id = municipios.estadoId AND municipios.id = :id');        
    ...
    public function getMunicipios(){
        $this->db->query('SELECT * FROM municipios ORDER BY nomeMunicipio ASC');
    ...
    public function getMunicipiosPag($page,$options){        
        ...
        //NOME DO MUNICIPIO
        if(!empty($options['named_params'][':nomeMunicipio']) && $options['named_params'][':nomeMunicipio'] != 'null'){            
            $this->pag->bindParam(':nomeMunicipio', $options['named_params'][':nomeMunicipio'], PDO::PARAM_STR, 12);  
        } 

         //ESTADO ID
        if(!empty($options['named_params'][':estadoId']) && $options['named_params'][':estadoId'] != 'null'){            
            $this->pag->bindParam(':estadoId', $options['named_params'][':estadoId'], PDO::PARAM_STR, 12);  
        }   
    ...
    public function getMunicipioByName($_nomeMunicipio){
        $this->db->query('SELECT * FROM municipios WHERE nomeMunicipio = :nomeMunicipio');        
    ...
    public function municipioExistente($_nomeMunicipio,$_estadoId){
        $this->db->query('SELECT * FROM municipios WHERE nomeMunicipio = :nomeMunicipio AND estadoId = :estadoId');        
        $this->db->bind(':nomeMunicipio', $_nomeMunicipio);
    ...
    public function deletaBairrosMunicipio($_municipioId){
        $this->db->query('DELETE FROM bairros WHERE municipioId = :municipioId');
